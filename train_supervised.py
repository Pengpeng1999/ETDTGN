...
if USE_MEMORY and MEMORY_DIM != node_features.shape[1]:
  logger.warning("Adjusting memory_dim from %d to %d to match node feature dimension.",
...
for i in range(args.n_runs):
  ...
  logger.info('Loading saved ETDTGN encoder')
  model_path = f'./saved_models/{args.prefix}-{DATA}.pth'
  load_result = tgn.load_state_dict(torch.load(model_path), strict=False)
  if len(load_result.missing_keys) > 0 or len(load_result.unexpected_keys) > 0:
    logger.warning('Non-strict state_dict load. missing_keys=%s unexpected_keys=%s',
                   load_result.missing_keys, load_result.unexpected_keys)
  tgn.eval()
  logger.info('ETDTGN encoder loaded')
