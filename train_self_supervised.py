...
parser = argparse.ArgumentParser('ETDTGN training')
parser.add_argument('-d', '--data', type=str, help='Dataset name (eg. wikipedia or reddit, mooc,lastfm,reddit)',
                    default='wikipedia')
...
parser.add_argument('--aggregator', type=str, default="type_based",
                    choices=["last", "mean", "type_based"],
...
parser.add_argument('--n_types', type=int, default=2,
                    help='Number of message types (k) for type_based aggregator')
...
if USE_MEMORY and MEMORY_DIM != node_features.shape[1]:
...
for i in range(args.n_runs):
  ...
  tgn = TGN(neighbor_finder=train_ngh_finder, node_features=node_features,
            ...
            dyrep=args.dyrep, n_types=args.n_types)
  ...
  for epoch in range(NUM_EPOCH):
    ...
    if USE_MEMORY:
      ...
      tgn.reset_dynamic_topology_state()
    ...
    if USE_MEMORY:
      ...
      train_memory_backup = tgn.backup_memory_state()
    ...
    val_ap, val_auc = eval_edge_prediction(model=tgn,
                                                            ...
                                                            n_neighbors=NUM_NEIGHBORS)
    if USE_MEMORY:
      val_memory_backup = tgn.backup_memory_state()
      # Restore memory we had at the end of training to be used when validating on new nodes.
      # Also backup memory after validation so it can be used for testing (since test edges are
      # strictly later in time than validation edges)
      tgn.restore_memory_state(train_memory_backup)
    ...
    if USE_MEMORY:
      ...
      tgn.restore_memory_state(val_memory_backup)
  ...
  logger.info('Saving ETDTGN model')
  if USE_MEMORY:
    # Restore memory at the end of validation (save a model which is ready for testing)
    tgn.restore_memory_state(val_memory_backup)
  torch.save(tgn.state_dict(), MODEL_SAVE_PATH)
  logger.info('ETDTGN model saved')
