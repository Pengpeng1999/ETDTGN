...
class Memory(nn.Module):
  ...
  def store_raw_messages(self, nodes, node_id_to_messages):
    for node in nodes:
      ...
      if len(self.messages[node]) > 0:
        self.nodes_with_messages.add(node)

  def get_nodes_with_messages(self):
    return list(self.nodes_with_messages)
  ...
  def restore_memory(self, memory_backup):
    ...
    self.nodes_with_messages = set(self.messages.keys())

  def detach_memory(self):
    self.memory.detach_()

    # Detach all stored messages
    for k in self.nodes_with_messages:
      v = self.messages[k]
      new_node_messages = []
      for message in v:
  ...
  def clear_messages(self, nodes):
    for node in nodes:
      ...
      self.nodes_with_messages.discard(node)
